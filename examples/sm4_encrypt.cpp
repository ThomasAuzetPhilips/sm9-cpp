...
int main (int ac, char*av[]) {
	...
	if (ac != 2) {
		std::cout << "Usage: " << av[0] << " <file_to_encrypt>" << std::endl;
		return -1;
	}
	std::vector<unsigned char> clear_buffer = file2buffer_(av[1]);
	std::vector<unsigned char> enc_buffer;
	switch (sm9::sm4_encrypt(clear_buffer, key, iv, enc_buffer)) {
		case sm9::OK:
	...
	if (!buffer2file_("out.enc", enc_buffer)) {
